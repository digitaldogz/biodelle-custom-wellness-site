      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-md overflow-hidden">
          ...
          <div className="bg-biodelle-beige/30 h-2">
            <div 
              className="bg-biodelle-terracotta h-full transition-all duration-300 ease-out"
              ...
              <form onSubmit={handleSubmit}>
                  ...
                  <div className="space-y-4">
                    {quizOptions.map((option) => (
                      <div
                        ...
                        className={cn(
                          "quiz-option",
                          selectedOption === option.id && "border-biodelle-terracotta bg-biodelle-offwhite"
                        )}
                  ...
                  <div className="space-y-4">
                    <div className="flex space-x-4">
                      <button 
                        type="button"
                        className={cn(
                          "px-6 py-2 border-2 rounded-md transition-all",
                          supplements ? "border-biodelle-moss bg-biodelle-moss text-white" : "border-gray-300"
                        )}
                        onClick={() => setSupplements("Sim")}
                      >
                        Sim
                      </button>
                      <button 
                        ...
                        className={cn(
                          "px-6 py-2 border-2 rounded-md transition-all",
                          supplements === "Não" ? "border-biodelle-moss bg-biodelle-moss text-white" : "border-gray-300"
                        )}
                        onClick={() => setSupplements("Não")}
                      >
                        Não
                      </button>
                    </div>
                    ...
                    {supplements === "Sim" && (
                      <div className="animate-fade-in mt-4">
                        ...
                        <textarea
                          className="w-full border border-gray-300 rounded-md p-3 focus:outline-none focus:ring-2 focus:ring-biodelle-terracotta"
                          rows={3}
                          placeholder="Por favor, descreva os produtos que você usa atualmente..."
                        ></textarea>
                      </div>
                    )}
                  </div>
                ...
                <div className={cn(
                  ...
                  <div className="mb-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {["Vegano", "Sem Glúten", "Sem Fragrância", "Hipoalergênico"].map((restriction) => (
                        <label
                          key={restriction}
                          className={cn(
                            "flex items-center p-3 border-2 rounded-md cursor-pointer transition-all",
                            restrictions.includes(restriction) ? "border-biodelle-moss bg-biodelle-offwhite" : "border-gray-200"
                          )}
                        >
                          <input
                            type="checkbox"
                          ...
                          <span className="w-5 h-5 mr-3 border-2 rounded flex items-center justify-center">
                            {restrictions.includes(restriction) && (
                              <span className="w-3 h-3 bg-biodelle-moss rounded-sm"></span>
                            )}
                          </span>
                          {restriction}
                        </label>
                    ...
                    <div className="mt-3">
                      ...
                      <input
                        ...
                        type="text"
                        className="w-full border border-gray-300 rounded-md p-3 focus:outline-none focus:ring-2 focus:ring-biodelle-terracotta"
                        placeholder="Especifique qualquer outra restrição..."
                    ...
                    <div className="flex items-start">
                      <div className="flex items-center h-5">
                        <input
                          id="terms"
                          type="checkbox"
                          className="w-4 h-4 border border-gray-300 rounded bg-gray-50 focus:ring-3 focus:ring-biodelle-terracotta"
                          required
                          checked={agreeTerms}
                ...
                <div className="mt-8 flex flex-wrap gap-3 justify-between">
                  {currentStep > 1 && (
                    <button
                      type="button"
                      className="border border-biodelle-moss text-biodelle-moss px-6 py-2 rounded-md transition-all hover:bg-biodelle-moss/10"
                      onClick={handleBack}
                    >
                      Voltar
                    </button>
                  ...
                  <div className="ml-auto">
                    {currentStep < 3 && (
                      <button
                        ...
                        onClick={handleNext}
                        disabled={(currentStep === 1 && !selectedOption) || (currentStep === 2 && !supplements)}
                      >
                        Próximo
                      </button>
                    )}
                    {currentStep === 3 && (
                      <button
                        type="submit"
                        className="biodelle-btn-primary flex items-center"
                        disabled={!name || !whatsapp || !agreeTerms || isSubmitting}
                      >
                        {isSubmitting ? (
                          <>
